...
The source project only supplies the definition. Direct conversations run in their own managed directory without the source project's files, instructions, Git state, terminal, or checkpoint controls. Skills that require the source workspace cannot use it from a direct conversation. Codex needs a version supporting named permission profiles; Claude needs its filesystem sandbox available.
