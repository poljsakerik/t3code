...
4. sends failed checks and the full review feedback back to the implementer when changes are needed;
...
T3 Code collects the entire review round before asking the implementer to assess the feedback and make changes. If every reviewer approves, verification completes without an extra agent turn.
